...
while( my $line =<$info>){
	...
	$ses_md5_path =~ s/_/\//g;
	my $filename="data/".$ses_md5_path."/".$md5p."_".$md5s;
	foreach $kval (keys(@array2)){
		my $var = $array2[$kval];
		if ( $var =~ /gen/ ){
			my $gen=substr($var,index($var,"_")+1);
			my $col=substr($gen,index($gen,"_")+1);
			my $currentfilename="fix/".$md5p."_".$md5s."_".$col."_generate";
			$currentfilename=$filename."_".$col."_generate" unless (-e $currentfilename);
